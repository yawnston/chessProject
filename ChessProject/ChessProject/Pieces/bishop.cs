...
namespace ChessProject.Pieces
{
    class bishop:piece
    {
        ...
        {
            //get bishop moves from the util (reused from resolving checks)
            List<square> consideredMoves = utils.bishopSliderMoves(board.squareArray[this.xPosition, this.yPosition]);
            //throw out moves that would put the bishop on a friendly piece
        ...
        public bishop(int x, int y, string owner)
        {
            xPos = x;
            yPos = y;
            type = "bishop";
            color = owner;
            if (color == "white") img = Image.FromFile("bishop-white.png");
            if (color == "black") img = Image.FromFile("bishop-black.png");
        }
    }
}
